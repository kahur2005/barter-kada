...
select ok((select prosrc like '%activeProductCount%' and prosrc like '%public.listings%' from pg_catalog.pg_proc where oid = 'public.get_my_stores()'::regprocedure), 'owner DTO exposes server-counted active products');

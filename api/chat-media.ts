...
export async function processChatMedia(
  input: { authorization: string | null; assetId: string },
  dependencies: ChatMediaDependencies,
  ...
  try {
    ...
    await dependencies.markProcessed({ assetId: input.assetId, actorId, processedPath: context.processedPath, width: processed.width, height: processed.height, contentHash: processed.contentHash });
...
function createDependencies(): ChatMediaDependencies {
  ...
  return {
    ...
    async getContext(assetId, actorId) {
      const { data, error } = await admin.rpc('get_chat_asset_processing_context', { p_asset_id: assetId, p_actor_id: actorId });
      if (error || !data || typeof data !== 'object') return null;
      const value = data as Record<string, unknown>;
      return typeof value.quarantinePath === 'string' && typeof value.processedPath === 'string' ? value as AssetContext : null;
    },
    async download(path) {
      const { data, error } = await admin.storage.from('chat-quarantine').download(path);
      if (error || !data) throw new Error('STORAGE_DOWNLOAD_FAILED');
      return Buffer.from(await data.arrayBuffer());
    },
    async uploadProcessed(path, body, mimeType) {
      const { error } = await admin.storage.from('chat-media').upload(path, body, { contentType: mimeType, cacheControl: '60', upsert: false });
      if (error) throw new Error('STORAGE_UPLOAD_FAILED');
    },
    async markProcessed(input) {
      const { error } = await admin.rpc('mark_chat_asset_processed', {
    ...
    async removeQuarantined(path) { await admin.storage.from('chat-quarantine').remove([path]); },
    async markRejected(assetId, actorId) { await admin.rpc('reject_chat_asset', { p_asset_id: assetId, p_actor_id: actorId }); },
  };
}
...
async function readJson(request: IncomingMessage & { body?: unknown }): Promise<Record<string, unknown>> {
  ...
  for await (const chunk of request) {
    const value = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk); length += value.byteLength;
    if (length > 4_096) throw new Error('REQUEST_TOO_LARGE'); chunks.push(value);
  }
...
export default async function handler(request: IncomingMessage & { body?: unknown }, response: ServerResponse) {
  ...
  try {
    ...
    const result = await processChatMedia({ authorization: typeof request.headers.authorization === 'string' ? request.headers.authorization : null, assetId: typeof body.assetId === 'string' ? body.assetId : '' }, createDependencies());
    response.statusCode = result.status; response.end(JSON.stringify(result.body));
  } catch {
    response.statusCode = 503; response.end(JSON.stringify({ error: 'MEDIA_SERVICE_UNAVAILABLE' }));
  }
}

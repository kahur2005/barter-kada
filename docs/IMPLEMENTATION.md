...
Bukti: `src/features/discovery/supabase-repository.test.ts`, `src/features/discovery/preview-repository.test.ts`, dan `src/app/App.test.tsx`; `npm.cmd test -- --run` menghasilkan 48 file/169 tes lulus, `npm.cmd run build` lulus, dan E2E preview menghasilkan 26 lulus/1 dilewati. Setelah persetujuan pengguna, migration `20260912141105_jabodetabek_service_area_boundaries.sql` mengimpor 13 area Jabodetabek resmi dari layer BIG DISIGT, tidak termasuk Kepulauan Seribu, ke project Supabase `barter`. Verifikasi runtime menunjukkan 13 area aktif, 13 boundary `MultiPolygon` SRID 4326, dan 13 boundary valid; REST `service_areas` dapat dibaca client dan RPC `search_listings` untuk `depok` mengembalikan empty state yang valid karena belum ada listing. Metadata sumber menyimpan layer dan versi query (`maxAllowableOffset=0.0001`).

...
export function StoreDetailPage() {
  ...
  return <div className="store-page"><Link className="back-link" to={returnPath(location.state, '/stores')}><Icon name="back" />Kembali ke hasil</Link><section className="store-profile"><div className="store-avatar large"><Icon name="shop" /></div><div><p className="metadata">Toko UMKM sekitar</p><h1>{data.name}</h1><p>{data.description}</p><p>{data.rating ? `${data.rating.toLocaleString('id-ID')} dari 5 (${data.reviewCount} ulasan)` : 'Belum ada ulasan'}</p></div></section>

...
const initialDraft: ListingDraft = {
  listingId: null, expectedVersion: null, publisher: { kind: 'personal' }, modes: ['sale'], fulfillment: 'ready_stock', categoryId: '', title: '', description: '', condition: null, defects: '', negotiable: false, barter: null, basePriceRupiah: null, variants: [], assetIds: [], handoverMethods: [], preorder: null, catering: null,
};
...
export function ListingEditorPage() {
  ...
  async function saveDraft() {
    ...
    try { const result = await gateway.saveDraft(draft); setDraft(current => ({ ...current, listingId: result.listingId, expectedVersion: result.version })); setDirty(false); setNotice('Draft tersimpan. Penawaran belum diterbitkan.'); }
  ...
  async function publish() {
    ...
    try { const result = await gateway.publish(draft); setDraft(current => ({ ...current, listingId: result.listingId, expectedVersion: result.version })); setDirty(false); setNotice('Penawaran berhasil diterbitkan dan dapat ditemukan warga sekitar.'); }
  ...
  return <div className="listing-editor"><header><Link className="back-link" to="/">Kembali</Link><p className="eyebrow">Jual, barter, atau bagikan</p><h1>Pasang penawaran</h1><p>Isi bertahap. Penawaran yang valid langsung terbit tanpa menunggu persetujuan admin.</p></header>
    ...
    <button className="text-button draft-save" type="button" onClick={saveDraft} disabled={!gateway || pending}>{gateway ? pending ? 'Menyimpan…' : 'Simpan draft' : 'Simpan draft tidak aktif'}</button>
  </div>;
}

...
export function MyListingsPage() {
  ...
  return <section className="my-listings"><header><div><p className="eyebrow">Kelola dagangan dan barang pribadi</p><h1>Listing saya</h1></div><Link className="button" to="/listings/new">Pasang baru</Link></header>
    ...
    <div className="owner-list">{data?.items.map(item => <article key={item.listingId}><div><span className="label">{tabs.find(candidate => candidate.key === item.lifecycle)?.label}</span><h2>{item.title || 'Draft tanpa nama'}</h2><p>Diperbarui {new Intl.DateTimeFormat('id-ID', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'Asia/Jakarta' }).format(new Date(item.updatedAt))}</p></div><div className="owner-actions"><Link className="button secondary" to={`/listings/${item.listingId}/edit`}>Edit</Link>{item.lifecycle === 'active' && <button className="button secondary" aria-label={`Arsipkan ${item.title}`} disabled={item.reserved || pendingId === item.listingId} onClick={() => archive(item)}>Arsipkan</button>}</div>{item.reserved && <p className="reservation-note">Selesaikan atau batalkan transaksi aktif sebelum mengedit detail barang eksklusif atau mengarsipkannya.</p>}</article>)}</div>
  </section>;
}

...
export function AdminAnalyticsPage() {
  ...
  return <section className="admin-analytics-page">
    ...
    {query.isPending ? <StatusPanel title="Memuat metrik…" /> : query.error || !query.data ? <StatusPanel title="Metrik tidak dapat dimuat" error><p>Pastikan rentang tanggal valid dan akunmu memiliki role admin aktif.</p><button className="button" type="button" onClick={() => void query.refetch()}>Coba lagi</button></StatusPanel> : <>
      <div className="metrics-grid"><article><span>Rata-rata respons chat</span><strong>{seconds(query.data.averageChatResponseSeconds)}</strong></article><article><span>Retensi D7</span><strong>{percent(query.data.retention.d7Rate)}</strong><small>{query.data.retention.d7Users} dari {query.data.retention.cohortUsers} akun cohort</small></article><article><span>Retensi W1</span><strong>{percent(query.data.retention.w1Rate)}</strong><small>{query.data.retention.w1Users} akun aktif kembali</small></article><article><span>Store aktif</span><strong>{query.data.activeStoreCount}</strong><small>{query.data.activePlusUserCount} pengguna Plus aktif</small></article></div>

...
it('renders a scoped admin report queue', async () => {
  const gateway: AdminGateway = { listReports: vi.fn().mockResolvedValue({ items: [report], nextCursor: null }), getReport: vi.fn(), decide: vi.fn(), getPlanSettings: vi.fn(), updatePlanLimits: vi.fn(), listPlanSettingsHistory: vi.fn(), getProductMetrics: vi.fn() };

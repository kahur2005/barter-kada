...
export function ReviewListPage() {
  ...
  return <section className="review-list-page"><PageHeading leading={<Link className="back-link" to={isStore ? '/stores' : '/'}>Kembali</Link>} kicker={isStore ? 'Reputasi toko' : 'Reputasi komunitas'} title={isStore ? 'Ulasan toko' : 'Ulasan'} description="Ulasan yang tampil berasal dari transaksi yang selesai dan telah memenuhi aturan publikasi." />{items.length ? <div className="review-list">{items.map(item => <article className="review-card" key={item.id}><header><strong>{item.authorName}</strong><span className="star-rating" aria-label={`${item.rating} dari 5 bintang`}>{Array.from({ length: 5 }, (_, i) => <Icon key={i} name="star" size={14} className={i < item.rating ? 'star-filled' : 'star-empty'} />)}</span></header><p>{item.comment || 'Tanpa komentar.'}</p><time dateTime={item.createdAt}>{new Intl.DateTimeFormat('id-ID', { dateStyle: 'medium', timeZone: 'Asia/Jakarta' }).format(new Date(item.createdAt))}</time>{item.reply ? <div className="review-reply"><strong>Balasan pemilik</strong><p>{item.reply.body}</p></div> : item.canReply ? replyId === item.id ? <div className="stack-form"><label htmlFor={`reply-${item.id}`}>Balasan<textarea id={`reply-${item.id}`} rows={3} maxLength={1000} value={body} onChange={event => setBody(event.target.value)} /></label>{(error || reply.error) && <p className="form-alert" role="alert">{error ?? 'Balasan belum tersimpan.'}</p>}<div className="form-actions"><button className="button secondary" type="button" onClick={() => setReplyId(null)}>Batal</button><button className="button" type="button" disabled={reply.isPending} onClick={submitReply}>Balas</button></div></div> : <button className="text-button" type="button" onClick={() => { setReplyId(item.id); setBody(''); setError(null); }}>Balas ulasan</button> : null}</article>)}</div> : <StatusPanel title="Belum ada ulasan"><p>Reputasi akan tampil setelah transaksi selesai.</p></StatusPanel>}</section>;
}

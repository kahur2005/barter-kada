...
export function ReviewListPage() {
  const gateway = useReviewGateway(); const [params] = useSearchParams(); const subjectId = params.get('subjectId') ?? ''; const client = useQueryClient();
  const query = useQuery({ queryKey: ['reviews', subjectId], queryFn: () => gateway!.list(subjectId, null), enabled: Boolean(gateway && subjectId) });
  ...
  if (!subjectId) return <StatusPanel title="Profil ulasan tidak ditemukan"><p>Tautan ulasan harus memuat profil yang ingin dilihat.</p></StatusPanel>;

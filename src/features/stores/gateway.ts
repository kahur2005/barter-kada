...
const storeSchema: z.ZodType<StoreSummary> = z.object({ id: z.string().uuid(), slug: z.string(), name: z.string(), description: z.string(), category: z.string(), areaId: z.string().optional(), areaLabel: z.string(), operatingHours: z.string().optional(), handoverMethods: z.array(z.string()).optional(), publicAddress: z.string().nullable().optional(), publicAddressConsent: z.boolean().optional(), activeProductCount: z.number().int().nonnegative().optional(), status: z.enum(['active', 'hidden']), updatedAt: z.string().datetime().optional() });
export interface StoreGateway { getPlusStatus(): Promise<PlusStatus>; createBillingOrder(method: BillingOrder['method']): Promise<BillingOrder>; simulateBilling(id: string, result: Exclude<BillingOrder['status'], 'pending'>): Promise<{ id: string; status: BillingOrder['status']; active: boolean }>; createStore(input: StoreInput): Promise<StoreSummary>; updateStore(id: string, input: StoreInput): Promise<StoreSummary>; getMyStores(): Promise<StoreSummary[]>; }
...
export function createSupabaseStoreGateway(client: SupabaseClient): StoreGateway {
  const storePayload = (input: StoreInput) => ({ p_name: input.name, p_description: input.description, p_category: input.category, p_area_id: input.areaId, p_area_label: input.areaLabel, p_operating_hours: input.operatingHours, p_handover_methods: input.handoverMethods, p_public_address: input.publicAddress, p_public_address_consent: input.publicAddressConsent });
  return {
    ...
    async createStore(input) { const { data, error } = await client.rpc('create_store', { p_slug: input.slug, ...storePayload(input) }); const parsed = storeSchema.safeParse(data); if (error || !parsed.success) throw failure(); return parsed.data; },
    async updateStore(id, input) { const { data, error } = await client.rpc('update_store', { p_store_id: id, ...storePayload(input) }); const parsed = storeSchema.safeParse(data); if (error || !parsed.success) throw failure(); return parsed.data; },

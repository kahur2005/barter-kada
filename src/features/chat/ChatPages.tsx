...
export function ChatInboxPage() {
  ...
  return <section className="chat-inbox"><BackButton fallback="/profile" /><header><p className="eyebrow">Percakapan privat</p><h1>Pesan</h1><p>Negosiasi tetap terpisah untuk setiap listing.</p></header>{query.data?.length ? <div className="conversation-list">{query.data.map(item => <Link key={item.id} to={`/chat/${item.id}`} aria-label={`${item.counterpart.name}, ${item.listing.title}`}><div><strong>{item.counterpart.name}</strong><span>{item.listing.title}</span><p>{item.lastMessage?.text ?? 'Belum ada pesan'}</p></div><div className="conversation-meta">{item.lastMessage && <time>{time(item.lastMessage.sentAt)}</time>}{item.unreadCount > 0 && <span>{item.unreadCount} belum dibaca</span>}<Icon name="arrow" /></div></Link>)}</div> : <StatusPanel title="Belum ada percakapan"><p>Buka sebuah listing lalu hubungi pemiliknya.</p><ActionLink to="/">Jelajahi listing</ActionLink></StatusPanel>}</section>;
}

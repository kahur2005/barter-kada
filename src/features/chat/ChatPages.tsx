...
import { BackButton, BackLink, ActionLink } from '../../components/NavigationLinks';
...
export function ChatInboxPage() {
  ...
  if (!gateway) return <section className="chat-inbox"><BackButton fallback="/profile" /><StatusPanel title="Pesan belum aktif"><p>Hubungkan backend untuk memakai percakapan nyata.</p></StatusPanel></section>;
  if (query.isPending) return <section className="chat-inbox"><BackButton fallback="/profile" /><StatusPanel title="Memuat pesan…" /></section>;
  if (query.error) return <section className="chat-inbox"><BackButton fallback="/profile" /><StatusPanel title="Pesan belum dapat dimuat" error><button className="button" onClick={() => void query.refetch()}>Coba lagi</button></StatusPanel></section>;
  return <section className="chat-inbox"><PageHeading kicker="Percakapan privat" title="Pesan" description="Negosiasi tetap terpisah untuk setiap listing." leading={<BackButton fallback="/profile" />} />{query.data?.length ? <div className="conversation-list">{query.data.map(item => <Link key={item.id} to={`/chat/${item.id}`} aria-label={`${item.counterpart.name}, ${item.listing.title}`}><div><strong>{item.counterpart.name}</strong><span>{item.listing.title}</span><p>{item.lastMessage?.text ?? 'Belum ada pesan'}</p></div><div className="conversation-meta">{item.lastMessage && <time>{time(item.lastMessage.sentAt)}</time>}{item.unreadCount > 0 && <span>{item.unreadCount} belum dibaca</span>}<Icon name="arrow" /></div></Link>)}</div> : <StatusPanel title="Belum ada percakapan"><p>Buka sebuah listing lalu hubungi pemiliknya.</p><ActionLink to="/">Jelajahi listing</ActionLink></StatusPanel>}</section>;
}
...
export function ChatRoomPage() {
  ...
  const previewUrls = useMemo(() => images.map(file => ({ file, url: URL.createObjectURL(file) })), [images]);
  useEffect(() => {
    return () => {
  ...
  const send = useMutation({
    mutationFn: (body: string) => gateway!.sendText(id, body, crypto.randomUUID()),
    onSuccess: () => {
      setText('');
      void client.invalidateQueries({ queryKey: ['chat', id, 'messages'] });
      void client.invalidateQueries({ queryKey: ['chat', 'inbox'] });
    },
    onError: () => {
  ...
  const sendImages = useMutation({
    mutationFn: (files: File[]) => gateway!.sendImages(id, files, crypto.randomUUID(), setImageProgress),
    onSuccess: () => {
      setImages([]);
      setImageProgress(0);
    ...
    onError: () => {
      showToast('Gagal mengirim foto. Silakan coba lagi.', 'error');
    }
  });
  const block = useMutation({
    mutationFn: () => gateway!.blockUser(conversation!.counterpart.id),
    onSuccess: () => {
      setConfirmBlock(false);
      setBlocked(true);
      showToast('Pengguna telah diblokir', 'info');
    }
  });
  const unblock = useMutation({
    mutationFn: () => gateway!.unblockUser(conversation!.counterpart.id),
    onSuccess: () => {
      setBlocked(false);
      showToast('Blokir dibuka', 'info');
    }
  });
  ...
  function submit(event?: FormEvent) {
    ...
    if (body && body.length <= 2_000 && !send.isPending) {
      send.mutate(body);
    }
  }
  ...
  if (!gateway) return <section className="chat-room"><BackButton fallback="/chat" /><StatusPanel title="Chat belum aktif"><p>Hubungkan backend untuk memakai percakapan nyata.</p></StatusPanel></section>;
  if (messages.isPending || inbox.isPending) return <StatusPanel title="Memuat percakapan…" />;
  if (messages.error || inbox.error || !conversation) return <StatusPanel title="Percakapan tidak tersedia" error><BackLink to="/chat">Kembali ke pesan</BackLink></StatusPanel>;
  return <section className="chat-room"><header className="chat-header"><BackLink to="/chat" /><div><h1>{conversation.counterpart.name}</h1><ActionLink className="chat-listing-link" to={`/listings/${conversation.listing.id}`}>{conversation.listing.title}</ActionLink></div><div className="chat-header-actions">{listing.data?.publisher.id === auth.session?.userId && <ActionLink className="text-button" to={`/orders/new/${id}`}>Buat ringkasan pesanan</ActionLink>}<ActionLink className="text-button" to={`/reports/new?targetType=conversation&targetId=${id}`}>Laporkan</ActionLink>{!blocked && !confirmBlock && <button className="text-button" type="button" onClick={() => setConfirmBlock(true)}>Blokir</button>}{confirmBlock && <div className="block-confirm" role="group" aria-label="Konfirmasi blokir"><button className="text-button danger" type="button" disabled={block.isPending} onClick={() => block.mutate()}>Ya, blokir</button><button className="text-button" type="button" onClick={() => setConfirmBlock(false)}>Batal</button></div>}</div></header>
    <aside className="chat-privacy-note" role="note" aria-label="Privasi lokasi"><span aria-hidden="true"><Icon name="lock" /></span><p><strong>Lokasi tepat tetap privat.</strong> Bagikan alamat hanya setelah detail penyerahan disepakati.</p></aside>
    ...
    {blocked ? <div className="blocked-notice" role="status"><p>Pengguna diblokir. Pesan baru dihentikan.</p><button className="text-button" type="button" disabled={unblock.isPending} onClick={() => unblock.mutate()}>{unblock.isPending ? 'Membuka…' : 'Buka blokir'}</button></div> : (
      <form className="chat-composer" onSubmit={submit}>
        {previewUrls.length > 0 && (
          <div className="chat-previews-strip" aria-label="Pratinjau foto yang akan dikirim">
            {previewUrls.map((item, index) => (
              <div key={item.url} className="chat-preview-item">
                <img src={item.url} alt={`Pratinjau ${index + 1}`} />
                <button
                  type="button"
        ...
        <div className="chat-media-controls">
          <label className="text-button" htmlFor="chat-images">Tambahkan foto</label>
          <input className="sr-only" id="chat-images" type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={chooseImages} />
          {images.length > 0 && (
            <>
              <span>{images.length} foto dipilih</span>
              <button className="text-button" type="button" disabled={sendImages.isPending} onClick={() => sendImages.mutate(images)}>
                {sendImages.isPending ? `Mengunggah ${imageProgress}%` : `Kirim ${images.length} foto`}
              </button>
              <button className="text-button" type="button" onClick={() => setImages([])}>Batal</button>
            </>
          )}
        </div>
        ...
        />
        <button className="button" disabled={!text.trim() || send.isPending}>{send.isPending ? 'Mengirim…' : 'Kirim'}</button>
      </form>
    )}
  </section>;
}
...
export function OpenConversationPage() {
  ...
  if (!gateway) return <section className="status-panel"><BackButton fallback={`/listings/${listingId}`} /><StatusPanel title="Chat belum aktif"><p>Hubungkan backend untuk memakai percakapan nyata.</p></StatusPanel></section>;
  return error ? <StatusPanel title="Percakapan tidak dapat dibuka" error><BackLink to={`/listings/${listingId}`}>Kembali ke listing</BackLink></StatusPanel> : <StatusPanel title="Membuka percakapan…" />;
}

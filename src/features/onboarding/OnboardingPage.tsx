...
function ProfileStep({ state, save }: { state: OnboardingState; save: (next: OnboardingState) => void }) {
  ...
  return <section aria-labelledby="profile-title"><h2 id="profile-title">Perkenalkan dirimu</h2><p>Nama ini terlihat oleh warga lain. Nomor WhatsApp dan alamat lengkap tetap privat.</p>
    ...
    <form className="stack-form" onSubmit={submit} noValidate>
      ...
      <button className="button" disabled={pending}>{pending ? 'Menyimpan…' : 'Simpan dan lanjut'}</button>
    </form>
  </section>;
}

function LocationStep({ areas, save }: { areas: ServiceArea[]; save: (next: OnboardingState) => void }) {
  const gateway = useOnboardingGateway()!;
  ...
  return <section aria-labelledby="location-title"><h2 id="location-title">Atur lokasi privat</h2><p>Koordinat dipakai untuk menghitung jarak maksimum 5 km. Pengguna lain hanya melihat area perkiraan.</p>
    {error && <p className="form-alert" role="alert">{error}</p>}
    <form className="stack-form" onSubmit={submit}>
      <label htmlFor="area">Wilayah</label><select id="area" name="areaId" defaultValue="" required><option value="" disabled>Pilih wilayah</option>{areas.map(area => <option key={area.areaId} value={area.areaId}>{area.name}</option>)}</select>
...
function PhoneStep({ save }: { save: (next: OnboardingState) => void }) {
  const gateway = useOnboardingGateway()!;
  ...
  async function request(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await gateway.requestOtp({ phone, purpose: 'register' });
  ...
  return <section aria-labelledby="phone-title"><h2 id="phone-title">Verifikasi WhatsApp</h2><p>Nomor dipakai untuk keamanan akun, bukan untuk login dan tidak dibagikan di listing.</p>
    {error && <p className="form-alert" role="alert">{error}</p>}{notice && <p className="success-notice" role="status">{notice}</p>}
    <form className="stack-form" onSubmit={request}><label htmlFor="phone">Nomor WhatsApp</label><input id="phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" placeholder="0812 3456 7890" required /><button className="button secondary" disabled={pending || waitSeconds > 0}>{pending ? 'Meminta…' : waitSeconds > 0 ? `Kirim ulang dalam ${waitSeconds} dtk` : challenge ? 'Kirim ulang kode' : 'Kirim kode'}</button></form>
...
export function OnboardingPage() {
  ...
  return <div className="onboarding-page"><header><p className="eyebrow">Satu akun, dua cara berjualan</p><h1>Lengkapi akun</h1><p>Selesaikan tiga tahap ini sebelum memasang barang, membuka toko Plus, mengobrol, atau bertransaksi.</p></header><Steps active={state.nextStep} />
    <div className="onboarding-panel">{state.nextStep === 'profile' && <ProfileStep state={state} save={setState} />}{state.nextStep === 'location' && <LocationStep areas={areas} save={setState} />}{state.nextStep === 'phone' && <PhoneStep save={setState} />}{state.nextStep === 'complete' && <section className="completion-panel"><h2>Akun siap digunakan</h2><p>Profil, lokasi privat, dan nomor {state.maskedPhone ?? 'WhatsApp'} telah lengkap.</p><Link className="button" to="/">Lihat penawaran sekitar</Link></section>}</div>
  </div>;
}
